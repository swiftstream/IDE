...
export async function updateSwiftDependencies(options: {
    type?: SwiftBuildType,
    force: boolean,
    ...
    // `swift package update` initiated by the official Swift extension
    if (DevContainerConfig.swiftVersion().major == 5) {
        await currentStream?.awaitForCompletionOfOtherSwiftProcessesIfNeeded('Updating dependencies')
    }
    const packageUpdate = async () => {
        if (!currentStream) { throw `stream is null` }
        await currentStream.swift.packageUpdate({
    ...
    if (!doesBuildFolderExists(options.type ?? SwiftBuildType.Native)) {
        print({
            detailed: `🔦 Updating Swift dependencies ${options.type ? `for ${options.type}` : ''}`,
            verbose: `🔦 Updating Swift dependencies ${options.type ? `at \`.${options.type}\`` : ''} for the first time`
        })
        buildStatus(`Updating dependencies`)
        await packageUpdate()
        saveLastModifiedDateForKey(LastModifiedDateType.SwiftPackage, options.type)
        measure.finish()
        print(`🔦 Updated in ${measure.fulltime}`, LogLevel.Detailed)
    }
